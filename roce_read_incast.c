...
void parseOpt(int argc, char **argv){
	int c;
	int i=0;
	while ((c = getopt(argc, argv, "s:m:n:c:b:h:p:e:M:d:S:")) != -1) {
		switch (c)
		{
			case 'm':
...
/*			case 'i':
			...
			case 'M':
				...
				break;
			default:
				fprintf(stderr, "usage: %s -m<message_size> -n<num_send_request> -s<signaled> -b<inter_port_no> -e<event_mode> -h<host_ip> -c<is_client> -p<port_no> -d<congestion_window_size> -M<num_threads>\n", argv[0]);
				exit(-1);
		}
	}
...
end_sending:
	...
	//printf("data %llu, %llu, %d\n", conn->num_sendcount, conn->send_message_size, conn->num_completions);
	double tput = (double)data*8.0/diff;
	double tot_time = (double)diff/BILLION;
	//printf("throughput @ thread %d time %llu ns, size %lu bits, tput %f Gb/s\n", index, diff, data*8, tput);
	printf("- [%.9g, %lu, %.9g, 1]\n", tot_time, data, tput);
...
int main(int argc, char **argv)
{
	...
	for(index=0;index<num_threads;index++){
		...
		/*	struct timespec _start, _end;
			...
			int i ; 
			for(i=0;i<100;i++){
				modify_qp(conn->qp, i%8, conn->remote_qp_attr);
			}
			clock_gettime(CLOCK_MONOTONIC, &_end);
			long long unsigned diff = (long long unsigned)(BILLION * (_end.tv_sec - _start.tv_sec) + _end.tv_nsec - _start.tv_nsec);
			printf("%llu\n", diff);
			exit(0);	*/
		}else{
			run_server(portno, conn, s_ctx->pd, index);
...
//	signal(SIGALRM, sigalarm_handler);
	if(is_client==1){
		...
		long unsigned data;
		for(index=0;index<num_threads;index++){
			s_ctx = multi_ctx[index];
			data += (long unsigned)s_ctx->conn->num_completions*s_ctx->conn->send_message_size;
		}
		double tput = (double)data*8.0/diff;
		double tot_time = (double)diff/BILLION;
