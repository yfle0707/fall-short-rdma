...
void parseOpt(int argc, char **argv){
	int c; 
	int i=0;	
	while ((c = getopt(argc, argv, "s:m:n:c:b:h:p:e:M:")) != -1) {
		switch (c)
		{
			case 'm':
...
/*			case 'i':
			...
			case 'M':
				...
				break;
			default:
				fprintf(stderr, "usage: %s -m<message_size> -n<num_send_request> -s<signaled>  -e<event_mode> -h<host_ip> -c<is_client> -p<port_no> -M<num_threads> -b<internal_port_no>\n", argv[0]);
				exit(-1);
		}
	}
...
static void * incast(void * arg){
	...
	//printf("tot_bytes: %"PRIu64", %d, %d \n", tot_bytes, conn->num_sendcount, conn->num_requests);
...
int main(int argc, char **argv)
{
	...
	for(index=0;index<num_threads;index++){
		...
		if(conn->isClient){   
			...
			if(connect_ctx(conn, conn->local_qp_attr->psn, conn->remote_qp_attr, COMMON_RONE)){
...
void run_server(int portno,struct connection *conn, struct ibv_pd *pd){
	...
	if(conn->send_transport == 2){
		...
		if(connect_ctx(conn, conn->local_qp_attr->psn, 
					conn->remote_qp_attr, COMMON_RONE)) {
